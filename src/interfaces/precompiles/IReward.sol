...
interface IReward {
    ...
    /// @dev Withdraws rewards in IMUA tokens earned from the dogfood AVS or from other AVSs that also distribute IMUA
    /// as rewards. Unlike `withdrawReward`, if the IMUA reward are from the dogfood AVS, this function sends the IMUA
    /// tokens directly
    ...
    /// similar to how other reward tokens are handled. In this case, the actual token transfer will not occur within
    /// the native module. If the IMUA rewards originate from multiple AVSs, including the dogfood AVS, this interface
    /// will return both
