...
describe("Precompile State Reversion Issue", () => {
    ...
    it("should demonstrate state persistence despite transaction revert", async () => {        
        ...
        const nonce1 = await reverterContract.nonce();
        console.log("Initial nonce:", nonce1);
        ...
        // Get intermediate balance (if it exists)
        ...
        const nonce2 = await reverterContract.nonce();
        expect(nonce2).to.equal(nonce1 + 1n, `Nonce should have increased by 1, previous: ${nonce1}, after: ${nonce2}`);
        ...
        const nonce3 = await reverterContract.nonce();
        if(nonce3 > nonce2) {
            console.log(chalk.red(`ISSUE CONFIRMED: Precompile Caller state is not reverted, nonce before revert call:${nonce2}, after: ${nonce3}`));
        } else {
            console.log(`Nonce reverted as expected`);
        }
        ...
        if (finalBalance > intermediateBalance) {
            console.log(chalk.red("ISSUE CONFIRMED: Precompile state change was not reverted!"));
            ...
            console.log(chalk.green("State was properly reverted"));
    ...
    it("should demonstrate try/catch catching the precompile revert", async () => {
        ...
        try {
            ...
            console.log(chalk.green("Outter try/catch call successfully catch the precompile error(revert)"));
        } catch (error) {
            console.log(chalk.red("Outter try/catch call cannot catch the precompile error(revert)"));
